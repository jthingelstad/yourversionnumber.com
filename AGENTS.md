...
4. **Person rows are display-only.** No inline inputs, no hover affordances, no click-to-edit. Editing is gated behind the header's Edit button → `<dialog>`. Rows render `.person-name` (when set) plus `.version`. Nothing else.
