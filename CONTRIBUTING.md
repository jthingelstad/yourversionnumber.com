...
Themes are the easiest way to contribute. Each one is a single CSS file. There's no JavaScript involved, no build step, changes are checked for appearance, accessibility, privacy, and compatibility.
...
3. **Open the page** with `npx live-server` and visit `http://localhost:8080/birthday/?theme=your-name&p=Jamie:1980-01-01` to see it.
...
- `.card-message`, `.card-recipient`, `.card-note`, `.card-from`
...
Group `.home-btn`, `.about-btn`, and `.edit-btn` in your selectors so both header buttons share the same look:

```css
.home-btn, .about-btn, .edit-btn { /* ... */ }
.home-btn:hover, .about-btn:hover, .edit-btn:hover { /* ... */ }
```
...
- [ ] Loads cleanly with `?theme=your-name` (no people), `?theme=your-name&p=:1980-01-01` (one unnamed person), `?theme=your-name&p=Jamie:1980-01-01&p=Sara:1982-03-15` (named, multiple).
...
- Pick a `home` for its ordering preference, or `null` to place it with the native choices in both editions.
